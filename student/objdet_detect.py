...
def load_configs_model(model_name='darknet', configs=None):
    ...
    print('configs.pretrained_filename:', configs.pretrained_filename)
...
def setup_configs_for_resnet(configs):
    configs.pin_memory = True
...
def create_resnet_model(configs):
...
def create_model(configs):
    ...
    if (configs.arch == 'darknet') and (configs.cfgfile is not None):
        ...
        setup_configs_for_resnet(configs)
        model = create_resnet_model(configs)
        #######
        ####### ID_S3_EX1-4 END #######     
